...
1"solve(grad): ";
\t x2:first .qml.solve[b{(x-y$;:[;enlist neg y])}'A;enlist n#0];

/ solve with optimization as least-squares problem
opts:`iter,1000000;
1"min:         ";
\t x3:first .qml.minx[opts][{x$x}b-A mmu;enlist n#0];
1"min(grad):   ";
\t x4:first .qml.minx[opts][({x$x}b-A mmu;enlist(-2*flip A)mmu b-A mmu);enlist n#0];
1"minx`nm:     ";
\t x5:first .qml.minx[`nm,opts][{x$x}b-A mmu;enlist n#0];
1"minx`sbplx:  ";
\t x6:first .qml.minx[`sbplx,opts][{x$x}b-A mmu;enlist n#0];

/ check results
if[1e-7<{max -1+(b%x)|x%b}A mmu x0;'`incorrect];
if[1e-3<max{max -1+(x0%x)|x%x0}each(x1;x2;x3;x4;x5;x6);'`different];
...
1"min(grad):   ";
\t x2:first .qml.minx[opts][({x$x}b-A mmu;enlist(-2*flip A)mmu b-A mmu);enlist n#0];
1"minx`nm:     ";
